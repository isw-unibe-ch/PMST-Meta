...
# Google Maps and Stadia Maps APIs - you have to add your API for the code to work
register_google(key = "YOUR-KEY", write = FALSE)
register_stadiamaps("YOUR-KEY", write = FALSE)
```
...
morphs_count_tap <- morphs_scen %>%
  distinct(morph, position, in_cells, .keep_all = TRUE) %>% 
